...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_order > 0 && in_array($status, ['pending', 'diproses', 'selesai', 'batal'])) {
        ...
        if ($row = mysqli_fetch_assoc($query)) {
            ...
            if ($current_status === 'batal') $allow_update = false;
            ...
            if ($allow_update) {
                ...
                if ($status === 'batal') {
                    $q_detail = mysqli_query($conn, "SELECT id_menu, jumlah FROM order_detail WHERE id_order = $id_order");
                    while ($detail = mysqli_fetch_assoc($q_detail)) {
                        mysqli_query($conn, "UPDATE menu SET stok = stok + {$detail['jumlah']} WHERE id_menu = {$detail['id_menu']}");
                    }
                }
            }
        }
